...
int main(int argc, char* argv[])
{
	...
	const string filename = argv[3]; //读入的文件名前缀
	...
	if (1 <= k && k <= 14)
	{
		...
		write_hash_table_less_than_14* write_hashtable_less_than_14 = new write_hash_table_less_than_14(argv, root_table);
	...
	else if (15 <= k && k <= 32)
	{
		...
		thread t1(generate_hash_table, argv, hash_table_1, hash_table_2, filename + ".HT1", filename + ".HT2");
		t1.join();
		...
		thread t3(deal_creads, argv, hash_table_1, hash_table_2, &print_list, &kmer_address_list, filename + ".creads", &run_flag); //处理序列提取与转换为字符串

		fstream char_key_value_file = fstream("Result.txt", ios::out | ios::binary);
		write_thread* print_key_values = new write_thread(argv, char_key_value_file, &print_list, &kmer_address_list, &run_flag); //同时进行写入
		t3.join();

		delete print_key_values; // 写入结束
		write_hash_table_15_32* write_hashtable_15_32 = new write_hash_table_15_32(argv, hash_table_1, hash_table_2, char_key_value_file);
	...
	else if (33 <= k && k <= 64)
	{
		...
		thread t1(generate_hash_table2, argv, hash_table_1, hash_table_2, filename + ".HT1", filename + ".HT2");
		t1.join();
		// cout << "size 1 = " << hash_table_1->size() << endl;
		// cout << "load factor 1 = " << hash_table_1->load_factor() << endl;
		// cout << "size 2 = " << hash_table_2->size() << endl;
		...
		CQ_kmer_block kmer_address_list; //回收存kmer的块
		bool run_flag = true; //用于释放空间
		thread t3(deal_creads2, argv, hash_table_1, hash_table_2, &print_list, &kmer_address_list, filename + ".creads", &run_flag); //处理序列提取与转换为字符串
		...
		write_hash_table_33_64* write_hashtable_33_64 = new write_hash_table_33_64(argv, hash_table_1, hash_table_2, char_key_value_file);
