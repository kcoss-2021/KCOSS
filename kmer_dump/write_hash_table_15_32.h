...
typedef moodycamel::ConcurrentQueue<key_value*> CQ_bin_key_value;
typedef moodycamel::ConcurrentQueue<char_key_value*> CQ_char_key_value;

class write_hash_table_15_32
{
public:
	write_hash_table_15_32(char** def_argv, HashTable1* hash_table_1_def, cuckoo_hash_map* hash_table_2_def, fstream& char_key_value_file_def);
	~write_hash_table_15_32();
	void read_table();
	void bin2char_block(key_value* block_address);
	void bin2char(char_key_value* fna, int& arry_flag, uint_64& kmer, uint_32& value);
	void write_table();

private:
	int k;
	int array_max;
	//CQ_bin_key_value CQ_bin_storage; //二进制的储存
	CQ_bin_key_value CQ_bin_recycling; //二进制块回收
...
write_hash_table_15_32::write_hash_table_15_32(char** def_argv, HashTable1* hash_table_1_def, cuckoo_hash_map* hash_table_2_def, fstream& char_key_value_file_def) :char_key_value_file(char_key_value_file_def)
{
	k = atoi(def_argv[1]);
...
write_hash_table_15_32::~write_hash_table_15_32()
{
	run = false;
	t_write.join();
}

inline void write_hash_table_15_32::read_table()
{
	int arry_flag = 0; //判断是否数组使用情况
	key_value* fna;
	fna = new key_value[array_max];
	uint_64 sum = 0;
	uint_64 value_sum = 0;
	auto* thread_pool = new ThreadPool_stable<key_value*>([&](key_value* block_address) { this->bin2char_block(block_address); }, 30);    ///d_hashtable1->insert_hashtable(block_address)
...
void write_hash_table_15_32::bin2char_block(key_value* block_address)
{
	//printf("????????????????????????????????????????????????????????");
...
inline void write_hash_table_15_32::bin2char(char_key_value* fna, int& arry_flag, uint_64& kmer, uint_32& value)
{
	for (int i = k - 1; i >= 0; i--)
	{
...
inline void write_hash_table_15_32::write_table()
{
	t_write = thread([&]
		{
			char_key_value* item;
