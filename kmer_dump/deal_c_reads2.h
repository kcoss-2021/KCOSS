...
{
public:
	deal_c_reads2(char** def_argv, HashTable2* hash_table_1, cuckoo_hash_map2* hash_table_2, CQ_kmer_block* print_list, CQ_kmer_block* kmer_address_list, Concurrent_Queue_c_reads* address_array)
	{
		k = atoi(def_argv[1]);
	...
	void execute(c_reads* block_address)
	{
		...
		const uint_320 ones_front = ~(~zeros << (2 * k - 64)); //截取前半截
		const uint_320 ones_end = ~(~zeros << 64); //截取后半截
		auto update_function = [](size_t& num) { ++num; };
		__uint128_t kmer = 0; //临时存放二进制的kmer
		__uint128_t tmp_128;
		...
		while ((array_num < array_max) && (block_address[array_num].mt != 0)) //小于块上限且非空
		{
			for (size_t i = 0; i < block_address[array_num].mt; i++)
			{
				...
				tmp_128 = ((block_address[array_num].creads >> 64) & ones_front).to_ullong();
				kmer = (tmp_128 << 64) + (block_address[array_num].creads & ones_end).to_ullong();
				...
				if (!(hash_table_1->upsertIfPresent(kmer, 1) || hash_table_2->update_fn(kmer, update_function)))
				{
					...
					if (arry_flag > (fna_array_max - 1)) //判断数组是否已满
					{
						...
						if (!kmer_address_list->try_dequeue(fna)) //1:item非空
						{
							...
							{
								while (!kmer_address_list->try_dequeue(fna))
								{
									usleep(20);
								}
							}
							...
							//{
							//	fna = mularr_new(fna_array_max, k + 1);
							//	cout << "creads2txt new" << endl;
							//}
							//catch (bad_alloc)
							//{
							//	cout << "creads2txt我没内存了" << endl;
							//	throw "creads2txt凉了";
							//}
	...
	void bin2str(char** fna, int& arry_flag, __uint128_t& kmer)
	{
		for (int i = k - 1; i >= 0; i--)
		{
			//cout << "i:" << i << endl;
