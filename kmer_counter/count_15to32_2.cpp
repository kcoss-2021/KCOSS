...
void count_15to32_2::count(char* reads_address)
{
    int arr_num = 0;
        ...
        else //拼接完整！！！！！！！！！！！！！！！！！
        {
            ...
            if (bloom_filter->test_set_64(kmer_tmp)) // 0:出现过 1:第一次出现
            {
                ...
                else // creads不再连续，将临时变量中的值处理然后新建
                {
                    ...
                    if (creads_Block[0] > 0)
                    {
                        creads_Block[creads_flag_loc] =
                                (creads_end << 16) + (creads_Block[0] - creads_flag_loc + 1);
                    }
